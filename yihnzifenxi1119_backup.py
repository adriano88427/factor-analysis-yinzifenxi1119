...
class FactorAnalysis:
    ...
    def generate_factor_analysis_report(self, summary_df, process_factors=False, factor_method='standardize', winsorize=False):
        ...
        # 首先为summary_df添加综合得分列，确保所有后续功能正常工作
        factor_scores = []
        factor_ratings = []
        factor_types = []
        
        for _, row in summary_df.iterrows():
            eval_result = self._evaluate_factor_performance(
                row['IC均值'], row['IC标准差'], row['IR值'],
                row.get('t统计量', np.nan), row.get('p值', np.nan), row.get('多空收益', np.nan)
            )
            factor_scores.append(eval_result['score'])
            factor_ratings.append(eval_result['rating'])
            factor_types.append(eval_result['factor_type'])
        ...
        summary_df['综合得分'] = factor_scores
        summary_df['评级'] = factor_ratings
        summary_df['因子类型'] = factor_types
        
        # 检查并输出实际的列名，以便调试
        print("Summary DataFrame 实际列名:", summary_df.columns.tolist())
        ...
        with open(report_filename, 'w', encoding='utf-8') as f:
            ...
            f.write(f"DEFAULT_DATA_FILE: {DEFAULT_DATA_FILE}\n")
            f.write("\n")
            ...
            for _, row in summary_df.iterrows():
                ...
                factor_rankings.append({
                    '因子名称': row['因子名称'],
                    '综合得分': eval_result['score'],
                    '评级': eval_result['rating'],
                    'IC均值': row['IC均值'],
                    'IR值': row['IR值'],
                    '多空收益': row.get('多空收益', np.nan),
                    'p值': row.get('p值', np.nan),
                    '因子类型': eval_result['factor_type'],
                    '使用建议': eval_result['usage']
                })
            
            # 转换为DataFrame
            ranking_df = pd.DataFrame(factor_rankings)
            
            # 将因子分为正向因子和负向因子两类
            positive_factors = ranking_df[ranking_df['IC均值'] > 0].copy()
            negative_factors = ranking_df[ranking_df['IC均值'] < 0].copy()
            
            # 添加综合得分列存在性检查并排序
            if '综合得分' in ranking_df.columns and not ranking_df['综合得分'].isna().all():
                positive_factors = positive_factors.sort_values('综合得分', ascending=False)
                negative_factors = negative_factors.sort_values('综合得分', ascending=False)
            else:
                print("警告: 因子排名数据中缺少'综合得分'列，跳过排序")
            
            # 输出正向因子排行榜
            f.write("【正向因子排行榜】\n")
            f.write("=" * 60 + "\n")
            f.write(f"{'排名':<4} {'因子名称':<25} {'得分':<6} {'评级':<6} {'IC均值':<8} {'IR值':<8}\n")
            f.write("-" * 65 + "\n")
            
            if len(positive_factors) > 0:
                for idx, (_, row) in enumerate(positive_factors.iterrows(), 1):
                    score_str = f"{row['综合得分']:<6.1f}" if '综合得分' in row and not pd.isna(row['综合得分']) else "N/A"
                    f.write(f"{idx:<4} {row['因子名称']:<25} {score_str} {row['评级']:<6} "
                           f"{row['IC均值']:<8.3f} {row['IR值']:<8.3f}\n")
                f.write(f"\n最佳正向因子: {positive_factors.iloc[0]['因子名称']} (排名第1)\n")
            else:
                f.write("未发现正向因子\n")
            ...
            if len(negative_factors) > 0:
                # 按IC均值的绝对值排序（负向因子IC均值越负绝对值越大排名越前）
                negative_factors_sorted = negative_factors.sort_values('IC均值', ascending=True)
                for idx, (_, row) in enumerate(negative_factors_sorted.iterrows(), 1):
                    score_str = f"{row['综合得分']:<6.1f}" if '综合得分' in row and not pd.isna(row['综合得分']) else "N/A"
                    f.write(f"{idx:<4} {row['因子名称']:<25} {score_str} {row['评级']:<6} "
                           f"{row['IC均值']:<8.3f} {row['IR值']:<8.3f}\n")
                best_negative = negative_factors_sorted.iloc[0]
                f.write(f"\n最佳负向因子: {best_negative['因子名称']} (排名第1，IC={best_negative['IC均值']:.3f})\n")
            else:
                f.write("未发现负向因子\n")
            ...
            if len(positive_factors) > 0:
                ...
                f.write("最佳正向因子推荐:\n")
                f.write(f"• 因子名称: {best_positive['因子名称']}\n")
                f.write(f"• 综合得分: {best_positive['综合得分']:.1f}/10\n")
                f.write(f"• IC均值: {best_positive['IC均值']:.3f}\n")
                f.write(f"• IR值: {best_positive['IR值']:.3f}\n")
                f.write(f"• 建议权重: 高权重配置（5%-15%）\n")
                f.write(f"• 使用策略: 因子值高时做多，因子值低时减仓\n\n")
            
            if len(negative_factors) > 0:
                best_negative = negative_factors.sort_values('IC均值', ascending=True).iloc[0]
                f.write("最佳负向因子推荐:\n")
                f.write(f"• 因子名称: {best_negative['因子名称']}\n")
                f.write(f"• 综合得分: {best_negative['综合得分']:.1f}/10\n")
                f.write(f"• IC均值: {best_negative['IC均值']:.3f}\n")
                f.write(f"• IR值: {best_negative['IR值']:.3f}\n")
                f.write(f"• 建议权重: 负权重配置（-5%至-10%）\n")
                f.write(f"• 使用策略: 反向使用，做空因子信号\n\n")
            ...
            for _, row in summary_df.iterrows():
                ...
                f.write(f"【{eval_result['rating']}级】{factor}\n")
                f.write("-" * 50 + "\n")
                f.write(f"综合得分: {eval_result['score']}/10\n")
                f.write(f"风险等级: {'低' if eval_result['score'] >= 8 else '中' if eval_result['score'] >= 6 else '高'}\n\n")
                
                # 核心指标
                f.write("核心指标:\n")
                f.write(f"• IC均值: {ic_mean:.3f}\n")
                f.write(f"• IR值: {ir:.3f}\n")
                # 格式化多空收益值
                ls_return_str = f"{long_short_return:.3f}" if not np.isnan(long_short_return) else "N/A"
                f.write(f"• 多空收益: {ls_return_str}\n")
                # 格式化p值
                p_value_str = f"{p_value:.3f}" if not np.isnan(p_value) else "N/A"
                f.write(f"• p值: {p_value_str}\n\n")
                
                # 添加详细理由说明和更全面的分析
                f.write("评测分析:\n")
                f.write(f"{eval_result['detailed_reason']}\n")
                
                # 计算各指标排名
                ic_mean_rank = (summary_df['IC均值'] >= ic_mean).sum()
                ir_rank = (summary_df['IR值'] >= ir).sum()
                ls_return_rank = len(summary_df)
                if not np.isnan(long_short_return):
                    ls_return_rank = (summary_df['多空收益'] >= long_short_return).sum()
                p_value_rank = len(summary_df)
                if not np.isnan(p_value):
                    p_value_rank = (summary_df['p值'] < p_value).sum() + 1
                ...
                if ic_mean < 0:
                    # 计算负向因子排名（绝对值）
                    negative_ic_rank = (summary_df['IC均值'] <= ic_mean).sum()
                    ...
                    if abs(ic_mean) >= 0.1:
                        negative_score += 4
                        negative_strength = "强负向"
                        negative_desc = "因子值越高收益越低"
                    elif abs(ic_mean) >= 0.07:
                        negative_score += 3
                        negative_strength = "中强负向"
                        negative_desc = "因子值与收益呈显著负相关"
                    elif abs(ic_mean) >= 0.05:
                        negative_score += 2
                        negative_strength = "中等负向"
                        negative_desc = "因子值越高收益越低"
                    elif abs(ic_mean) >= 0.03:
                        negative_score += 1
                        negative_strength = "弱负向"
                        negative_desc = "因子值与收益轻微负相关"
                    else:
                        negative_strength = "极弱负向"
                        negative_desc = "因子值与收益关系不明确"
                    
                    # 基于稳定性的评分（0-3分）
                    if ir > 1.5:
                        negative_score += 3
                        stability_desc = "稳定性优秀，可控性强"
                    elif ir > 1.0:
                        negative_score += 2
                        stability_desc = "稳定性良好"
                    elif ir > 0:
                        negative_score += 1
                        stability_desc = "稳定性一般"
                    else:
                        stability_desc = "稳定性较差"
                    
                    # 基于收益表现的评分（0-3分）
                    if not np.isnan(long_short_return):
                        if long_short_return < -0.02:
                            negative_score += 3
                            return_desc = "多空收益显著为负，适合反向策略"
                        elif long_short_return < -0.01:
                            negative_score += 2
                            return_desc = "多空收益为负，具有反向价值"
                        elif long_short_return < 0:
                            negative_score += 1
                            return_desc = "多空收益轻微为负"
                        else:
                            return_desc = "收益为正但因子为负向，可能存在数据问题"
                    else:
                        return_desc = "收益数据缺失"
                    
                    # 最终负向因子评分
                    final_negative_score = min(negative_score, 10)
                    
                    f.write(f"\n负向因子深度分析:\n")
                    f.write(f"• 该因子为负向因子（IC均值 < 0），与收益呈反向关系\n")
                    f.write(f"• 负向因子排名: 第{negative_ic_rank}名 (共{total_factors}个因子)\n")
                    f.write(f"• 负向因子综合评分: {final_negative_score}/10分\n")
                    
                    # 负向强度评估
                    f.write(f"• 负向强度评估: {negative_strength} - {negative_desc}\n")
                    
                    # 可用性详细分析
                    f.write(f"• 可用性评估:\n")
                    if final_negative_score >= 8:
                        f.write("  - 评级: A级（优秀反向因子）\n")
                        f.write("  - 可用性: 强烈推荐反向使用\n")
                        f.write("  - 建议权重: -10%至-20%（负权重）\n")
                        f.write("  - 交易策略: 直接反向建仓，做空因子信号\n")
                    elif final_negative_score >= 6:
                        f.write("  - 评级: B级（良好反向因子）\n")
                        f.write("  - 可用性: 推荐反向使用\n")
                        f.write("  - 建议权重: -5%至-10%（负权重）\n")
                        f.write("  - 交易策略: 反向建仓，注意仓位控制\n")
                    elif final_negative_score >= 4:
                        f.write("  - 评级: C级（一般反向因子）\n")
                        f.write("  - 可用性: 谨慎使用\n")
                        f.write("  - 建议权重: -3%至-5%（小仓位负权重）\n")
                        f.write("  - 交易策略: 结合其他正向因子组合使用\n")
                    else:
                        f.write("  - 评级: D级（不推荐使用）\n")
                        f.write("  - 可用性: 不建议使用\n")
                        f.write("  - 建议权重: 0%（不使用）\n")
                        f.write("  - 交易策略: 建议重新评估因子构建方法\n")
                    
                    # 稳定性分析
                    f.write(f"• 稳定性分析: {stability_desc}\n")
                    
                    # 收益表现分析
                    f.write(f"• 收益表现分析: {return_desc}\n")
                    
                    # 反向策略详细建议
                    f.write(f"• 反向策略建议:\n")
                    if not np.isnan(long_short_return):
                        if long_short_return < -0.015:
                            f.write("  - 策略: 激进反向策略\n")
                            f.write("  - 操作: 因子值高时大量做空，因子值低时平仓\n")
                            f.write("  - 预期收益: 年化-1.5%以上（反向收益）\n")
                        elif long_short_return < -0.01:
                            f.write("  - 策略: 标准反向策略\n")
                            f.write("  - 操作: 因子值高时做空，因子值低时减仓\n")
                            f.write("  - 预期收益: 年化-1%至-1.5%（反向收益）\n")
                        elif long_short_return < 0:
                            f.write("  - 策略: 保守反向策略\n")
                            f.write("  - 操作: 因子值极高时少量做空\n")
                            f.write("  - 预期收益: 轻微反向收益\n")
                        else:
                            f.write("  - 策略: 数据核查\n")
                            f.write("  - 操作: 暂停使用，检查数据质量\n")
                            f.write("  - 预期收益: 需要重新评估\n")
                    ...
                    # 组合使用建议
                    f.write(f"• 组合使用建议:\n")
                    if final_negative_score >= 6:
                        f.write("  - 核心策略: 负权重配置，与正向因子形成对冲\n")
                        f.write("  - 组合比例: 负权重因子占组合20-30%\n")
                        f.write("  - 对冲效果: 降低组合波动性，提升夏普比率\n")
                    elif final_negative_score >= 4:
                        f.write("  - 辅助策略: 小幅负权重，作为组合调整工具\n")
                        f.write("  - 组合比例: 负权重因子占组合5-10%\n")
                        f.write("  - 对冲效果: 轻微降低组合风险\n")
                    else:
                        f.write("  - 不建议组合使用\n")
                        f.write("  - 建议寻找替代的正向因子\n")
                ...
                # 增加详细的多维度分析
                f.write("\n详细指标分析:\n")
                f.write(f"• 预测能力评估: {ic_mean:.3f} (IC均值)\n")
                if ic_mean > 0.05:
                    f.write("  - 预测能力: 良好，IC均值超过0.05阈值\n")
                elif ic_mean > 0:
                    f.write("  - 预测能力: 一般，IC均值为正值\n")
                else:
                    f.write("  - 预测能力: 较差，IC均值为负值\n")
                f.write(f"  - 排名: 第{ic_mean_rank}名 (共{total_factors}个因子)\n")
                
                f.write(f"• 稳定性评估: {ir:.3f} (IR值)\n")
                if ir > 1.5:
                    f.write("  - 稳定性: 优秀，IR值超过1.5\n")
                elif ir > 1:
                    f.write("  - 稳定性: 良好，IR值超过1\n")
                elif ir > 0:
                    f.write("  - 稳定性: 一般，IR值为正\n")
                else:
                    f.write("  - 稳定性: 较差，IR值为负\n")
                f.write(f"  - 排名: 第{ir_rank}名 (共{total_factors}个因子)\n")
                
                f.write(f"• 收益表现: {ls_return_str} (多空收益)\n")
                if not np.isnan(long_short_return):
                    if long_short_return > 0.02:
                        f.write("  - 收益表现: 优秀，年化多空收益超过2%\n")
                    elif long_short_return > 0:
                        f.write("  - 收益表现: 一般，年化多空收益为正\n")
                    else:
                        f.write("  - 收益表现: 较差，年化多空收益为负或接近0\n")
                    f.write(f"  - 排名: 第{ls_return_rank}名 (共{total_factors}个因子)\n")
                else:
                    f.write("  - 收益表现: 数据缺失，无法评估\n")
                    f.write(f"  - 排名: N/A (共{total_factors}个因子)\n")
                
                f.write(f"• 统计显著性: {p_value_str} (p值)\n")
                if not np.isnan(p_value):
                    if p_value < 0.01:
                        f.write("  - 统计显著性: 高度显著 (p < 0.01)\n")
                    elif p_value < 0.05:
                        f.write("  - 统计显著性: 显著 (p < 0.05)\n")
                    elif p_value < 0.1:
                        f.write("  - 统计显著性: 边际显著 (p < 0.1)\n")
                    else:
                        f.write("  - 统计显著性: 不显著 (p >= 0.1)\n")
                    f.write(f"  - 排名: 第{p_value_rank}名 (共{total_factors}个因子)\n")
                else:
                    f.write("  - 统计显著性: 数据缺失，无法评估\n")
                    f.write(f"  - 排名: N/A (共{total_factors}个因子)\n")
                
                # 添加具体的使用建议
                f.write(f"\n具体使用建议:\n")
                if eval_result['score'] >= 3.0:
                    f.write("• 强烈推荐使用，可作为核心因子构建策略\n")
                    f.write("• 建议配置较高权重(15-25%)\n")
                    f.write("• 适合中长期持仓策略\n")
                elif eval_result['score'] >= 2.0:
                    f.write("• 推荐使用，建议与其他优质因子组合\n")
                    f.write("• 建议配置中等权重(8-15%)\n")
                    f.write("• 适合平衡型投资策略\n")
                elif eval_result['score'] >= 1.5:
                    f.write("• 谨慎使用，建议小仓位配置(3-8%)\n")
                    f.write("• 适合作为辅助因子使用\n")
                    f.write("• 需要密切监控其有效性变化\n")
                else:
                    f.write("• 不建议使用，可能带来负收益\n")
                    f.write("• 如必须使用，仓位应控制在3%以下\n")
                    f.write("• 建议重新评估因子构建方法\n")
                ...
                f.write("风险提示:\n")
                if eval_result['score'] >= 3.0:
                    f.write("• 低风险：可直接用于策略构建\n")
                elif eval_result['score'] >= 2.0:
                    f.write("• 中低风险：建议与其他因子组合使用\n")
                elif eval_result['score'] >= 1.5:
                    f.write("• 中等风险：谨慎使用，需要加强监控\n")
                else:
                    f.write("• 高风险：不建议使用，可能带来损失\n")
                
                if not np.isnan(p_value) and p_value >= 0.05:
                    f.write("• 统计风险：统计不显著，结果可能存在偶然性\n")
            ...
            f.write("本报告采用基于国内量化实践的优化评分体系，综合评估因子投资价值：\n\n")
            
            f.write("【评分体系说明】\n")
            f.write("-" * 30 + "\n")
            f.write("本评分体系针对正向因子和负向因子采用不同的权重配置：\n\n")
            
            f.write("正向因子权重配置（基于国内量化实践）：\n")
            f.write("• IC均值评分（35%权重，提升预测能力重要性）\n")
            f.write("• 统计显著性评分（25%权重，提升可靠性重视度）\n")
            f.write("• IR值评分（20%权重，降低稳定性权重）\n")
            f.write("• 多空收益评分（20%权重，降低收益权重）\n\n")
            
            f.write("负向因子专门评分体系（针对反向策略优化）：\n")
            f.write("• 负向强度评分（40%权重，|IC均值|越大越好）\n")
            f.write("• 统计显著性评分（30%权重，p值显著性）\n")
            f.write("• IR值稳定性（20%权重，稳定性表现）\n")
            f.write("• 收益表现评分（10%权重，多空收益）\n\n")
            
            f.write("【具体评分标准】\n")
            f.write("-" * 30 + "\n")
            
            f.write("一、正向因子IC均值评分（35%权重）\n")
            f.write("• ≥0.12：优秀（4.0分）+ 超额奖励机制\n")
            f.write("• ≥0.08：国内A级标准（3.5分）\n")
            f.write("• ≥0.05：国内B级标准（3.0分）\n")
            f.write("• ≥0.02：有效阈值（2.0分）\n")
            f.write("• ≥0.01：弱（1.0分）\n")
            f.write("• <0.01：极弱（0.5分）\n\n")
            
            f.write("二、统计显著性评分（正向25%，负向30%）\n")
            f.write("• p值<0.01：高度显著（1.0分）\n")
            f.write("• p值<0.05：显著（0.8分）\n")
            f.write("• p值<0.1：边缘显著（0.6分）\n")
            f.write("• p值≥0.1：不显著（0.3分）\n\n")
            
            f.write("三、IR值评分（正向20%，负向20%）\n")
            f.write("• ≥1.5：极强（正向2.5分，负向2.0分）\n")
            f.write("• ≥1.0：强（正向2.0分，负向1.5分）\n")
            f.write("• ≥0.5：中等（正向1.5分，负向1.0分）\n")
            f.write("• ≥0.3：弱（正向1.0分，负向0.8分）\n")
            f.write("• ≥0.15：较弱（正向0.8分，负向0.5分）\n")
            f.write("• <0.15：极弱（正向0.5分，负向0.5分）\n\n")
            
            f.write("四、多空收益评分（正向20%，负向10%）\n")
            f.write("• ≥4%：优秀（正向2.0分）\n")
            f.write("• ≥3%：强（正向1.8分）\n")
            f.write("• ≥2%：中等（正向1.5分）\n")
            f.write("• ≥1%：弱（正向1.0分）\n")
            ...
            f.write("五、负向因子专门评分\n")
            f.write("负向强度评分（40%权重）：\n")
            f.write("• |IC均值|≥0.10：强负向（4.0分）\n")
            f.write("• |IC均值|≥0.07：中强负向（3.5分）\n")
            f.write("• |IC均值|≥0.05：中等负向（3.0分）\n")
            f.write("• |IC均值|≥0.03：弱负向（2.0分）\n")
            f.write("• |IC均值|<0.03：极弱负向（1.0分）\n\n")
            
            f.write("负向因子收益表现评分（10%权重）：\n")
            f.write("• 多空收益<-2%：优秀反向收益（1.0分）\n")
            f.write("• 多空收益<-1%：良好反向收益（0.8分）\n")
            f.write("• 多空收益<0：一般反向收益（0.6分）\n")
            f.write("• 多空收益≥0：数据问题（0.3分）\n\n")
            
            f.write("【综合评级标准】\n")
            f.write("-" * 30 + "\n")
            f.write("基于加权总分（0-10分制）和国内量化实践标准的评级：\n\n")
            
            f.write("正向因子评级标准（结合国内实践）：\n")
            f.write("• A+级：IC均值≥0.08且IR≥0.3，总分≥3.5（卓越，强烈推荐使用）\n")
            f.write("• A级：IC均值≥0.08且IR≥0.3，总分≥3.0（优秀，推荐使用）\n")
            f.write("• A-级：IC均值≥0.08，总分≥2.5（优秀，特殊机制）\n")
            f.write("• B+级：IC均值≥0.05且IR≥0.2，总分≥2.5（良好，可考虑使用）\n")
            f.write("• B级：IC均值≥0.05且IR≥0.2，总分≥2.0（一般，谨慎使用）\n")
            f.write("• C+级：IC均值≥0.02，总分≥1.5（较弱，不推荐使用）\n")
            f.write("• C级：IC均值≥0.02，总分≥1.0（弱，不建议使用）\n")
            f.write("• D级：IC均值<0.02（无效，避免使用）\n\n")
            
            f.write("负向因子评级标准（基于绝对IC值和稳定性）：\n")
            f.write("• A-级：|IC均值|≥0.08且|IR|≥0.3（优秀反向因子）\n")
            f.write("• B+级：|IC均值|≥0.05且|IR|≥0.2（良好反向因子）\n")
            f.write("• B级：|IC均值|≥0.03（一般反向因子）\n")
            f.write("• C+级：|IC均值|≥0.02（较弱反向因子）\n")
            f.write("• D级：|IC均值|<0.02（无效，避免使用）\n\n")
            
            f.write("【特别评级机制】\n")
            f.write("-" * 30 + "\n")
            f.write("对于IC均值>0.12且多空收益>0.04的优秀因子：\n")
            f.write("• 特殊处理：即使总分较低也获得A-或A评级\n")
            f.write("• 超额奖励：基于绝对值超出的额外加分\n")
            f.write("• 权重建议：推荐15-25%配置权重\n\n")
            
            f.write("【使用建议】\n")
            f.write("-" * 30 + "\n")
            f.write("正向因子使用建议：\n")
            f.write("• A+和A级因子：作为组合核心配置，权重15-25%\n")
            f.write("• B+和B级因子：作为组合辅助配置，权重5-15%\n")
            f.write("• C级因子：谨慎使用，权重控制在5%以内\n")
            f.write("• D级因子：不建议使用\n\n")
            
            f.write("负向因子使用建议：\n")
            f.write("• A-级负向因子：强烈推荐反向使用，权重-10%至-20%\n")
            f.write("• B+级负向因子：推荐反向使用，权重-5%至-10%\n")
            f.write("• B级负向因子：谨慎反向使用，权重-3%至-5%\n")
            f.write("• C+级及以下：不建议使用\n\n")
            
            f.write("【国内量化实践标准】\n")
            f.write("-" * 30 + "\n")
            f.write("本评级体系采用国内量化行业实践标准：\n")
            f.write("• A级因子：IC均值>0.08，IR>0.3（行业优秀标准）\n")
            f.write("• B级因子：IC均值>0.05，IR>0.2（行业良好标准）\n")
            f.write("• 统计显著性：p值<0.05为显著，<0.01为高度显著\n")
            f.write("• 稳定性要求：IR值>0.5为基本稳定，>1.0为稳定\n")
        ...
        print(f"因子分析详细报告已保存到 '{report_filename}'")
